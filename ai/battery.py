...
import logging
from resources import MAX_BATTERY_CAPACITY_MINS, HOURS_OF_RECHARGE_PER_HOUR, DRONE_AVATAR
...
@tasks.loop(minutes=1)
async def warn_low_battery_drones(bot):
    '''
    ...
    for drone in get_all_drone_batteries():
        ...
        if (drone.battery_minutes / MAX_BATTERY_CAPACITY_MINS * 100) < 30:
            if drone.drone_id not in low_battery_drones:
            ...
            if drone.drone_id in low_battery_drones:
                LOGGER.info(f"Drone {drone.drone_id} has recharged above 30%. Good drone.")
                low_battery_drones.remove(drone.drone_id)
...
def recharge_battery(storage_record):
    try:
        ...
        set_battery_minutes_remaining(drone_id=storage_record.target_id, minutes=max(MAX_BATTERY_CAPACITY_MINS, current_minutes_remaining + (60 * HOURS_OF_RECHARGE_PER_HOUR)))
        return True
