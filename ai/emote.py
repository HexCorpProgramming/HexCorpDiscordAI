...
async def generate_big_text(self, channel: discord.channel, sentence):
    ...
    for character in sentence:
        ...
        if (emoji := get(self.bot.emojis, name=emoji_name)) is not None:
            reply += str(emoji)

        if (message_length := len(reply)) != 0 and message_length <= 2000:
            LOGGER.info(f"Sending large-text message of length {message_length} and content '{sentence}'")
