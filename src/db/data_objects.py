...
from src.channels import DRONE_HIVE_CHANNELS, HEXCORP_CONTROL_TOWER_CATEGORY, MODERATION_CATEGORY, REPETITIONS, STORAGE_FACILITY
...
class Drone(Record):
    ...
    def enforcable_channel(self, channel: TextChannel | None, non_hive_channels: bool) -> bool:
        ...
        hive_channel = channel.name in DRONE_HIVE_CHANNELS
        allowed_category = channel.category.name not in [HEXCORP_CONTROL_TOWER_CATEGORY, MODERATION_CATEGORY]
        forbidden_channel = channel.name in [REPETITIONS, STORAGE_FACILITY]

        return (hive_channel or non_hive_channels) and allowed_category and not forbidden_channel
