...
def delete_timers_by_drone_id_and_mode(drone_id: str, mode: str):
    '''
    Deletes the timer with the given ID and mode.
    '''
    change('DELETE FROM timer WHERE drone_id = :drone_id AND mode = :mode', {'drone_id': drone_id, 'mode': mode})
...
def get_timers_elapsed_before(time: datetime) -> List[Timer]:
    ...
    '''
    return map_to_objects(fetchall('SELECT id, drone_id, mode, end_time FROM timer WHERE end_time < :time', {'time': time}), Timer)
