...
from db.drone_dao import is_glitched, is_prepending_id, is_optimized, is_identity_enforced
...
async def update_display_name(user: discord.Member):
    ...
    new_display_name = f"{'⬢' if is_optimized(user) or is_glitched(user) or is_prepending_id(user) or is_identity_enforced(user) else '⬡'}-Drone #{drone_id}"
